...
namespace MetricCalculator.logic
{
    public class Parser
    {
        ...
        public List<string> BasicOperatorsWords = new List<string> {  "if", "else", "for", "while", "switch", "case",  "break", "continue", "return"};
        ...
        public void Tokenize(string input)
        {
            try
            {
                ...
                //от*бываем токены пометок спецсимволов
        ...
        public void UpdateTokens()
        {
            try
            {
                ...
                for (int i = 0; i < tokens.Count; i++)
                {
                    ...
                    if (token == "(")
                    {
                        ...
                        if (i > 0 && !BasicOperators.Contains(tokens[i - 1]))
                        {
                            isFunctionCall = true;
                        ...
                        {
                            throw new Exception($"Syntax error: Missing closing parenthesis for '(' at position {i}");
                        }
                        if (isFunctionCall)
                        {
                    ...
                    {
                        throw new Exception($"Syntax error: Unexpected closing parenthesis ')' at position {i}");
                    }
                    else if (token == "{")
                    {
                        int counter = i;
                        ...
                        if (counter >= tokens.Count || tokens[counter] != "}")
                        {
                            throw new Exception($"Syntax error: Missing closing brace for '{{' at position {i}");
                        }
                        tokens[counter] = " ";
                    }
                    else if (token == "}")
                    {
                        throw new Exception($"Syntax error: Unexpected closing brace '}}' at position {i}");
                    }
                    else if (token == "[")
                    {
                        int counter = i;
                        ...
                        if (counter >= tokens.Count || tokens[counter] != "]")
                        {
                            throw new Exception($"Syntax error: Missing closing bracket for '[' at position {i}");
                        }
                        tokens[counter] = " ";
                    }
                    else if (token == "]")
                    {
                        throw new Exception($"Syntax error: Unexpected closing bracket ']' at position {i}");
                    }
                }
                //удаление пробелов в токенах
                for (int i = 0; i < tokens.Count; i++)
                {
                    if (tokens[i] == " ")
                    {
            ...
            {
                MessageBox.Show($"Error updating tokens: {ex.Message}");
            }
